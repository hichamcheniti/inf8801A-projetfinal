...
big_size =  [size(im1,1), size(im1,2)]; % image dimensions
...
im_szX = size(im1,1); % width
im_szY = size(im1,2); % height
...
trg_mask = mask_Pyr{1}; %setting the target mask
...
im1 = cat(3,im1, zeros(size(im1,1),size(im1,2),2)); % initialize before hole filling
% performing hole filling on multiple iteration
for k = 1 : num_scales

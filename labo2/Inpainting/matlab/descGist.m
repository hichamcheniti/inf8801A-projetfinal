classdef descGist
   ...
   methods (Static)
       ...
       function filters = getGabors()
           % Pre-allocate 4D array of filters
           filters = zeros( descGist.filterSize, descGist.filterSize, descGist.nbScales, descGist.nbOri );

           for s = 1:descGist.nbScales
               scale = descGist.filterSize / 2^s;
               sigmaX = scale / 2;
               sigmaY = scale;
               lda = descGist.filterSize / 2;
               for o = 1:descGist.nbOri
                   theta = (pi / descGist.nbOri) * (o - 1);
                   filters(:, :, s, o) = descGist.getGabor(theta, lda, sigmaX, sigmaY);
               end
           end
       end
   ...
   methods
       ...
       function dst = descGist(src)
           % Uncomment to display filters
           %figure; descGist.displayFilters();
           ...
           for s = 1:descGist.nbScales
               for o = 1:descGist.nbOri
                   % Get response
                   imgResponse = imfilter(src, descGist.filters(:, :, s, o));
                   % Abs
                   imgResponse = abs(imgResponse);
                   % Scale down to a grid (method != 'nearest')
                   imgResponseGrid = imresize(imgResponse, [descGist.gridSize descGist.gridSize], 'bilinear');
                   dst.values(:, :, s, o) = imgResponseGrid;
               end
           end
       end
